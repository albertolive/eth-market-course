...
contract("CourseMarketplace", (accounts) => {
  ...
  describe("Purchase the new course", () => {
    ...
    it("can get the purchased course has by index", async () => {
      const index = 0;
    ...
    describe("Transfer ownership", () => {
      ...
      it("should transfer ownership back initial to contract owner", async () => {
  ...
  describe("Emergency withdraw", () => {
    ...
    it("should have +contract funds on contract owner", async () => {
      ...
      assert.equal(
        toBN(ownerBalance).add(toBN(contractBalance)).sub(toBN(gas)).toString(),
        newOwnerBalance,
        "Owner doesn't have contract balance!"
      );
    });

    it("should have contract balance of 0 after emergency withdraw", async () => {
      const contractBalance = await getBalance(_contract.address);
