...
const createFormState = (
  ...
  if (!price || Number(price) <= 0) {
    return _createFormState(true, "Price is required");
  }
  ...
  if (isNewPurchase) {
    if (confirmationEmail.length === 0 || email.length === 0) {
      ...
      return _createFormState(true, "Email and confirmation email must match");
    }
  }

  if (!hasAgreedTOS) {
    return _createFormState(true, "You must agree to the TOS");
  }
...
export default function OrderModal({
  ...
  useEffect(() => {
    if (!!course) {
      setIsOpen(true);
      setOrder({ ...defaultOrder, price: ethPrice.perItem });
    }
  }, [course]);
